...
def mutacion_gaussiana(cromosoma, sigma=0.1):
    ...
    for i in range(39):  # Por cada alumno
        idx = i * 3
        if random.random() < 0.1:  # Probabilidad de mutación por alumno
            # Aplicar ruido gaussiano a los 3 genes del alumno
            genes = [cromosoma_mutado[idx + j] + random.gauss(0, sigma) for j in range(3)]
            # Reemplazar negativos por 0
            genes = [max(0, g) for g in genes]
            suma = sum(genes)
            # Normalizar si la suma es válida, si no, usar distribución uniforme
            if suma > 0:
                genes = [g / suma for g in genes]
            else:
                genes = [1/3, 1/3, 1/3]
...
def algoritmo_genetico(generaciones=150, tam_poblacion=100):
    ...
    for gen in range(generaciones):
        ...
        while len(nueva_poblacion) < tam_poblacion:
            ...
            hijo = mutacion_gaussiana(hijo, sigma=1)  #PROBAR CON VARIOS VALORES DE SIGMA
