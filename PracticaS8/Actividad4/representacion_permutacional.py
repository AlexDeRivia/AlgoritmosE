...
indices_baja_nota = [i for i, nota in enumerate(notas) if nota < 11]
...
def calcular_fitness(cromosoma):
    ...
    # Verificar la restricción: no todos los de nota < 11 en un mismo grupo
    examen_con_todos = False
    for examen in ['A', 'B', 'C']:
        indices = asignaciones[examen]
        if all(i in indices for i in indices_baja_nota):
            examen_con_todos = True
            break
    if examen_con_todos:
        penalizacion_restriccion -= 0.5  # Penalización fuerte
    ...
    fitness = -desv_promedios + bonus_diversidad + penalizacion_restriccion
    return fitness
...
print("REPRESENTACION PERMUTACIONAL")
print("Problema: Secuenciar alumnos para asignacion ordenada a examenes")
print("Cromosoma: Permutacion de 39 indices de alumnos")
print("Decodificacion: Posiciones [0-12] -> Examen A, [13-25] -> Examen B, [26-38] -> Examen C\n")

mejor_solucion, historial = algoritmo_genetico()
asignaciones_finales = decodificar_cromosoma(mejor_solucion)

print("\nAsignacion final por orden de secuencia:")
for examen in ['A', 'B', 'C']:
    ...
    for i, idx in enumerate(indices):
        print(f"    Posicion {i+1}: {alumnos[idx]} (Nota: {notas[idx]})")
        if i >= 4:
...
# Mostrar distribución de alumnos con nota <11
print("\nDistribucion de alumnos con nota menor a 11:")
print(f"Total de alumnos con nota <11: {len(indices_baja_nota)}\n")

for examen in ['A', 'B', 'C']:
    indices = asignaciones_finales[examen]
    indices_baja = [i for i in indices if i in indices_baja_nota]
    
    print(f"Examen {examen}: {len(indices_baja)} alumnos con nota <11")
    for idx in indices_baja:
        print(f"  - {alumnos[idx]} (Nota: {notas[idx]})")
    if not indices_baja:
        print("  - Ninguno")
