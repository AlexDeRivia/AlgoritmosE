...
def calcular_fitness(cromosoma):
    ...
    for examen in ['A', 'B', 'C']:
        ...
        # Bonificación por diversidad (rango alto = buena mezcla de notas)
        if rango >= 6:
            bonificaciones_diversidad.append(0.1)
        elif rango >= 3:
            bonificaciones_diversidad.append(0.05)
        else:
            bonificaciones_diversidad.append(0)

    desviacion_promedios = np.std(promedios)
    penalizacion_varianza = sum(varianzas) / len(varianzas)
    bonificacion_total = sum(bonificaciones_diversidad)

    fitness = -desviacion_promedios - (0.05 * penalizacion_varianza) + bonificacion_total
    return fitness
...
def algoritmo_genetico(generaciones=100, tam_poblacion=50):
    ...
    for gen in range(generaciones):
        ...
        if gen % 20 == 0:
            ...
            print(f"Generacion {gen}: Mejor fitness = {mejor_fitness:.4f}")
    
    mejor_cromosoma = fitness_scores[0][0]
    return mejor_cromosoma

print("REPRESENTACION BINARIA")
print("Problema: Distribuir 39 alumnos en 3 examenes (A, B, C) de forma equitativa")
print("Cromosoma: 117 bits (39 alumnos x 3 bits cada uno)")
